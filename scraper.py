    ...
    if '500_csv' in link['href']:
        ...
        for l_csv in links_csv:
            if '/id/' in l_csv['href']:
                ...
                linkt = str(l_csv['href']).lower()
                msize = [m in linkt for m in months]
                msize = sum(msize)

                if msize >= 2:
                    url = l_csv['href']
                    csvfile = l_csv['href'].strip().replace('_', ' ').replace('/', ' ').split(' ')
                    csvYr = csvfile[10].strip()
                    csvMth = 'Q0'
                    data.append([csvYr, csvMth, url])
                else:
                    url = l_csv['href']
                    csvfile = l_csv['href'].strip().replace('_', ' ').replace('/', ' ').split(' ')
                    csvYr = csvfile[8].strip()
                    if csvYr == 'payments':
                        csvYr = '2013'
                    csvMth = csvfile[7][:3].strip()
